...
@main struct Main: AsyncParsableCommand {
  ...
  @Option(name: .long, help: "Number of cells per network.") var cellCount: Int = 64
  ...
  mutating func train() async throws {
    let loader = LogicGateLoader(stepCount: examplesPerRollout)
    ...
    while true {
      ...
      DispatchQueue.global(qos: .userInitiated).sync {
        DispatchQueue.concurrentPerform(iterations: solutions.count) { index in
          ...
          for (graph, (inputs, targets)) in zip(graphs, sequences) {
            ...
            let score = overlapScore(predictions: preds, targets: targets)
      ...
      print(
        "step \(step): fitness=\(finalFitnesses.reduce(0.0, +) / Double(finalFitnesses.count)) step_time=\(duration) min_fitness=\(finalFitnesses.reduce(1.0, min)) max_fitness=\(finalFitnesses.reduce(0.0, max))"
      )
    }
  }
}
